  ...
  const blogCategories = ['General',  'Technology', 'Business', 'Health', 'Lifestyle', 'Education', 'Travel', 'Food']
    ...
    const onSubmitHandler = async (e)=>{
      ...
      try {
        ...
        const prompt = `Generate the blog titles for the keyword ${input} in the category ${selectedCategory}. The tone should be empathetic, informative. Make sure the exact phrase ${input} is naturally integrated into the title.`
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
          ...
          <div className='flex items-center gap-3'>
            <Sparkles className='w-6 text-[#8E35EB]'/>
          ...
          <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-linear-to-r from-[#C341F6] to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
            ...
            :
            <Hash className='w-5'/>}  {/* // icon display */}
            Generate title
          </button>
      </form>
